...
{ 
  ...
  // done is 1 only if all processes are done.
  int done = 0;
  while (!done) {
    done = 1;
    for (int i = 0; i < n; i++) {
      // if process i not done checker
      if (rem_bt[i] > 0) {
        done = 0;
...
{ 
  ...
  int current_time = 0, completion_time = 0;
  
  while (process_counter < n) {
    // first find the process with minimum remaining time
    int min_i = 0;
    for (int i = 0; i < n; i++) {
      if (rem_bt[i] < rem_bt[min_i]) {
        min_i = i;
      }
    }
    ...
    if (rem_bt[min_i] == 0) {
      ...
      plist[min_i].wt = completion_time - plist[min_i].bt;
      rem_bt[min_i] = INT_MAX;
    }
